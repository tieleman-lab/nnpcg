...
class EasyTrainer():
    ...
    def train_command(self):
        ...
        print('python nnpcg/train.py --conf nnpcg/train.yaml --log-dir nnpcg/data/train_light')
...
class EasySimulator():
    ...
    def build_system(self, temperature=300):
        ...
        self.system.set_positions(self.mol.coords)  # Set the atomic positions for the system
        self.system.set_box(self.mol.box)  # Set the box dimensions for the system
        self.system.set_velocities(maxwell_boltzmann(self.parameters.masses, T=temperature, replicas=1))  # Set velocities for the system using Maxwell-Boltzmann distribution
    ...
    def run_simulation(self, logger_path="logs/", logger_name="monitor.csv", timestep=1, steps=1000, output_period=10, save_period=100, trajectoryout="trajectory.npy"):
        ...
        Epot = self.forces.compute(self.system.pos, self.system.box, self.system.forces)  # Calculate the potential energy of the system
        for i in iterator:
            Ekin, Epot, T = self.integrator.step(niter=output_period)  # Perform simulation step and calculate energy and temperature
